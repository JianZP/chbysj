      ...
      <div class="layui-card">
        ...
        <div class="layui-card-body">
          <div class="test-table-operate-btn" style="margin-bottom: 10px;">
            <button class="layui-btn" data-type="add" style="margin-left: 0px">添加设备类型信息</button>
            ...
            <div class="layui-inline">
              <input class="layui-input" name="keyword" id="keyword" autocomplete="off" placeholder="请输入设备名称">
            </div>
...
<div class="layui-row" id="uploadfile" style="display:none;">
  <div class="layui-col-md10">
    <form method="post"  enctype="multipart/form-data" id="form1" action="http://localhost:8080/fileUpload">
  ...
  <div class="layui-col-md10">
    <form class="layui-form layui-from-pane" action="" style="margin-top:20px" id="myform" >
      <div class="layui-form-item">
        ...
        <div class="layui-input-block">
          <input type="text" name="name"  required  lay-verify="required" autocomplete="off" placeholder="请输入设备名称" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
        <label class="layui-form-label">设备类型</label>
        <div class="layui-input-block">
          <input type="text" name="equiptype"  required  lay-verify="required" autocomplete="off" placeholder="请输入设备类型" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
        <label class="layui-form-label">设备型号</label>
        <div class="layui-input-block">
          <input type="text" name="typenumber"  required  lay-verify="required" autocomplete="off" placeholder="请输入设备型号" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
        <label class="layui-form-label">使用范围</label>
        <div class="layui-input-block">
          <input type="text" name="userange"  required  lay-verify="required" autocomplete="off" placeholder="请输入使用范围" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item" style="margin-top:40px">
        <div class="layui-input-block">
  ...
  layui.config({
    ...
    table.render({
      ...
      ,cols: [[
        ...
        ,{field:'typenumber', width:200, title: '设备型号'}
        ,{field:'userange', width:200, title: '使用范围'}
    ...
    table.on('tool(test-table-operate)', function(obj){
      ...
      if(obj.event === 'detail'){
        ...
        layer.confirm('确定删除吗？', function(index){
          $.ajax({
            url:'http://localhost:8080/delEquipType',
            method:'post',
            data:{id:data.id},
            dataType:'JSON',
    ...
    var $ = layui.$, active = {
      ...
      ,getCheckLength: function(){ //获取选中数目
        ...
        dom.src=" http://localhost:8080/ExcelDownloads?ids="+JSON.stringify(arr);
    ...
    layui.form.on('submit(sub)', function(data){
      $.ajax({
        url:'http://localhost:8080/insEquipType',
        method:'post',
        data:data.field,
        dataType:'JSON',
        success:function(res){
